...
import { Alert, Grid, SvgIcon, Typography } from '@mui/material';
import { updateProcessAutojump } from '../../store/actions/statusLedActions';
import { green, grey, red } from '@mui/material/colors';

const CustomAlert = ({ text, setOpenNotify }) => {
    useEffect(() => {
...
const ControlButtons = () => {
    ...
    const infraStatus = useSelector(state => state.statusled.sensorStatus["Infra"]);
    ...
    const currentBatch = useSelector(state => state.statusled.singleStatus['singleCurrentBatch']);
    ...
    const handleChangeAutoJump = (event) => {
        fetch('http://localhost:8000/api/update/process', {
            ...
            body: JSON.stringify({ AutoJump: !autoJump })
        })
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
                dispatch(updateProcessAutojump({ autojump: !autoJump }));
    ...
    const validateArray = () => {
        if (array.name === "") {
            ...
            for (let i = 0; i < array.length; i++) {
                ...
                // Iterate through each key in the element
                for (let key in element) {
                    if (element.hasOwnProperty(key)) {
                        // Check if the property value is zero or negative
                        if (element[key] <= 0) {
                            return false;
                        }
                    }
    ...
    const validateSingleValues = () => {
        for (let key in singleModeStatus) {
            if (singleModeStatus.hasOwnProperty(key)) {
                // Check if the property value is zero or negative
                if (singleModeStatus[key] <= 0) {
                    return false;
                }
            }
        }
    ...
    const handleStartWithCare = () => {
        if (Mode) {
            if (validateArray()) {
                handleStart()
            }
            else {
                setOpenNotify([true, "Invalid array"])
            }
        }
        else {
            if (validateSingleValues()) {
                ...
                setOpenNotify([true, "Invalid single mode values"])
            }
        }
    }

    const handleToggleInfra = () => {
        fetch('http://localhost:8000/api/set_infra', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ state: infraStatus ? "OFF" : "ON" })
        })
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
            ...
            <Grid>
                <Box sx={{ display: 'flex', justifyContent: 'center', gap: 2, marginBottom: 5 }}>
                    ...
                    {isAdmin &&
                        <Button
                            variant="contained"
                            onClick={handleToggleInfra}
                            disabled={isProcessStarted}
                            sx={{
                                width: 80, height: 80,
                                color: infraStatus ? "black" : "white",
                                fontWeight: 'bold',
                                backgroundColor: infraStatus ? "white" : red[900],
                                '&:hover': { backgroundColor: infraStatus ? "white" : red[900] }
                            }}
                        >
                            {infraStatus && (
                    ...
                    <Button
                        ...
                        sx={{
                            width: 80, height: 80,
                            color: autoJump ? "black" : "white",
                            fontWeight: 'bold',
                            backgroundColor: autoJump ? "white" : red[900],
                            '&:hover': { backgroundColor: autoJump ? "white" : red[900] }
                        }}
                    >
                        {autoJump && (
                    ...
                    <Button onClick={handleStop1} variant="contained" disabled={!isProcessStarted}
                        sx={{
                            width: 100, height: 80,
                            color: "white",
                            fontWeight: 'bold',
